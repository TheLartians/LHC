...
struct ArrayHasher {
  template <class Array> std::size_t operator()(const Array &a) const {
    ...
    size_t seed = 0;
    for (auto &v : a)
      hash_combine(seed, scalar_hasher(v));
    return seed;
  }
};
...
struct FloatHasher {
  size_t operator()(const float &f) const { return (unsigned &)f; }
};

template <class Array, class ScalarHasher = std::hash<typename Array::Scalar>>
struct NDArrayHasher {
  ScalarHasher scalar_hasher;
  std::size_t operator()(const Array &a) const {
    size_t seed = 0;
    a.for_all_values([&](const typename Array::Scalar &s) {
      seed ^= scalar_hasher(s) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
    });
    return seed;
  }
};
