...
// we create our own complex class since std::complex seems to be extremely slow
// (3x slower than the class below on our hardware, probably because operations
// are not inlined)
...
template <class Real> struct Complex {
  ...
  Complex operator*(const Scalar &other) const {
  ...
  Complex operator/(const Complex &other) const {
    ...
    return Complex((real * other.real + imag * other.imag) / d,
                   (-real * other.imag + imag * other.real) / d);
  }
...
template <class Real>
Complex<Real> operator+(const Real &s, const Complex<Real> &c) {
  return c + s;
}
template <class Real>
Complex<Real> operator-(const Real &s, const Complex<Real> &c) {
  return Complex<Real>(s, 0) - c;
}
template <class Real>
Complex<Real> operator*(const Real &s, const Complex<Real> &c) {
  return c * s;
...
template <class Real> Complex<Real> sqrt(const Complex<Real> &z) {
  return std::sqrt<Real>(z);
}
template <class Real>
Complex<Real> pow(const Complex<Real> &z1, const Complex<Real> &z2) {
  return std::pow<Real>(z1, z2);
}

template <class Real>
std::ostream &operator<<(std::ostream &stream, const Complex<Real> &c) {
  stream << '(' << c.real << ',' << c.imag << ')';
  return stream;
}

template <class Real>
std::wostream &operator<<(std::wostream &stream, const Complex<Real> &c) {
  stream << '(' << c.real << ',' << c.imag << ')';
  return stream;
}
