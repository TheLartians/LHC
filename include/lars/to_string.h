...
template <class T>
std::string stream_to_string(const T &obj, int precision = 0) {
...
template <typename T>
std::wstring stream_to_wstring(const T &value, int precision = 0) {
...
template <>
inline std::wstring stream_to_wstring<std::string>(const std::string &value,
                                                   int precision) {
  std::wostringstream stream;
  if (precision)
    stream.precision(precision);
