...
template <class Rep, class I>
std::string duration_to_string(const std::chrono::duration<Rep, I> &dur) {
  ...
  if (h.count() > 1)
    return lars::stream_to_string(h.count()) + " h";
  if (m.count() > 1)
    return lars::stream_to_string(m.count()) + " m";
  if (s.count() > 1)
    return lars::stream_to_string(s.count()) + " s";
  if (ms.count() > 1)
    return lars::stream_to_string(ms.count()) + " ms";
  if (us.count() > 1)
    return lars::stream_to_string(us.count()) + " us";
...
std::basic_ostream<Char> &operator<<(std::basic_ostream<Char> &stream,
                                     const TimeItResult &result) {
  stream << "average of " << result.repititions << ": "
         << duration_to_string(result.average_duration());
  return stream;
}

template <class Char>
std::basic_ostream<Char> &operator<<(std::basic_ostream<Char> &stream,
                                     const ExtendedTimeItResult &result) {
  stream << '[';
...
template <typename F>
ExtendedTimeItResult
time_it(const F &f, std::chrono::high_resolution_clock::duration timeout =
  ...
  result.worst = result.best = result.most_repititions =
      time_it(f, repititions);
  repititions *= 2;
  end = std::chrono::high_resolution_clock::now();

  while (end - begin < timeout) {
    result.most_repititions = time_it(f, repititions);
    if (result.most_repititions.average_duration() >
        result.worst.average_duration())
      result.worst = result.most_repititions;
    if (result.most_repititions.average_duration() <
        result.best.average_duration())
      result.best = result.most_repititions;
    if (repititions * 2 < repititions)
      break;
    repititions *= 2;
    end = std::chrono::high_resolution_clock::now();
  }
