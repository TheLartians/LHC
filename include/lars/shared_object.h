...
template <class T> class shared_object {
  ...
  void take_hash(const shared_object &other) {
...
public:
  ...
  shared_object(Args... args) : data(std::make_shared<T>(args...)) {}
  shared_object(const shared_object &other) : data(other.data) {
    take_hash(other);
  }
  ...
  template <typename Other> shared_object &operator=(Other other) {
    *data = other;
    return *this;
  }
  ...
  bool operator!=(const shared_object &other) const {
    if (hash_valid && other.hash_valid)
      if (cached_hash != other.cached_hash)
        return true;
    return *data != T(other);
  }
  ...
  template <typename Other> shared_object &operator+=(const Other &other) {
    *data += T(other);
    return *this;
  }
  template <typename Other> shared_object &operator*=(const Other &other) {
...
template <class T>
shared_object<T> operator+(const shared_object<T> &lhs,
                           const shared_object<T> &rhs) {
  return shared_object<T>((*lhs) + (*rhs));
}
template <class T>
shared_object<T> operator-(const shared_object<T> &lhs,
                           const shared_object<T> &rhs) {
...
template <class T>
std::ostream &operator<<(std::ostream &stream, const shared_object<T> &obj) {
  return stream << *obj;
}
