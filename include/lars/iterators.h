...
public:
  ...
  public:
    ...
    const_iterator operator++(int) {
      const_iterator range_copy(*this);
      current += increment;
      return range_copy;
    }
    const_iterator &operator--() {
    ...
    const_iterator &operator+=(const int &v) {
      current += v * increment;
      return *this;
    }
    const_iterator operator+(const int &v) {
      const_iterator range_copy(*this);
      range_copy += v * increment;
      return range_copy;
    }
    const_iterator &operator-=(const int &v) {
      current -= v * increment;
    ...
    bool operator!=(const const_iterator &rhs) {
      return (increment >= 0 && (current < rhs.current)) ||
             (increment < 0 && (current > rhs.current));
    }
  };

  range_wrapper(const T &r_start, const T &r_end, const T &r_increment)
      : r_start(r_start), r_end(r_end), r_increment(r_increment) {}
  ...
  const_reverse_iterator rbegin() const {
    return const_reverse_iterator(r_end - 1, -r_increment);
  }
  const_reverse_iterator rend() const {
    return const_reverse_iterator(r_start - 1, -r_increment);
  }

  range_wrapper<T> operator+(const T &d) {
    return range_wrapper<T>(r_start + d, r_end + d, r_increment);
  }
  range_wrapper<T> operator*(const T &m) {
    return range_wrapper<T>(r_start * m, r_end * m, r_increment * m);
  }
};

template <class T>
range_wrapper<T> range(const T &start, const T &end, const T &increment) {
  return range_wrapper<T>(start, end, increment);
}
template <class T> range_wrapper<T> range(const T &start, const T &end) {
  return range_wrapper<T>(start, end, 1);
}
template <class T> range_wrapper<T> range(const T &end) {
  return range_wrapper<T>(0, end, 1);
}

template <class C> range_wrapper<size_t> indices(const C &c) {
  return range<size_t>(c.size());
}
...
public:
  ...
  string_ref(value_type const *const begin, int const size)
...
std::vector<string_ref<string_type>>
split(const string_type &str,
  ...
  for (auto it = str.begin(); it != str.end(); ++it) {
    ...
    if (newState != state) {
      ...
      case inSpace:
        result.push_back(
            string_ref<string_type>(pTokenBegin, &*it - pTokenBegin));
        break;
      case inToken:
        pTokenBegin = &*it;
      }
  ...
  if (state == inToken)
    result.push_back(
        string_ref<string_type>(pTokenBegin, &str.back() - pTokenBegin + 1));
  return result;
}
...
public:
  ...
  public:
    ...
    iterator(unsigned UN, unsigned _size, bool end = false)
        : N(std::min(UN, size)), size(_size) {
      init(N, _size, end);
    }
    ...
    virtual bool step() { return increment_index(N - 1); }
    ...
    const std::vector<unsigned> &operator*() const { return positions; }
...
public:
  subarray_indices(unsigned N, unsigned size)
      : bit(iterator(N, size)), eit(iterator(N, size, true)) {}

  template <class T>
  subarray_indices(unsigned N, T &arr)
      : bit(iterator(N, arr.size())), eit(iterator(N, arr.size(), true)) {}
...
public:
  ...
  public:
    iterator() = default;
    iterator(unsigned N, unsigned size, bool end = false)
        : subarray_indices::iterator(N, size, end) {}

    bool step() override {
      if (std::next_permutation(positions.begin(), positions.end()))
        return true;
...
public:
  permutated_subarray_indices(unsigned N, unsigned size)
      : bit(iterator(N, size)), eit(iterator(N, size, true)) {}

  template <class T>
  permutated_subarray_indices(unsigned N, T &arr)
      : bit(iterator(N, arr.size())), eit(iterator(N, arr.size(), true)) {}
...
public:
  ...
  bool operator!=(const enumerate_iterator<iterator> &other) const {
    return it != other.it;
  }
};
...
public:
  enumerate_iterator_wrapper(const iterator &begin, const iterator &end)
      : bit(begin), eit(end) {}
...
template <typename T>
enumerate_iterator_wrapper<typename T::iterator> enumerate(T &arr) {
  return enumerate_iterator_wrapper<typename T::iterator>(arr.begin(),
                                                          arr.end());
}

template <typename T>
enumerate_iterator_wrapper<typename T::const_iterator> enumerate(const T &arr) {
  return enumerate_iterator_wrapper<typename T::const_iterator>(arr.begin(),
                                                                arr.end());
}
...
public:
  slice_wrapper(const iterator &begin, const iterator &end)
      : bit(begin), eit(end) {}
  iterator begin() const { return bit; }
  iterator end() const { return eit; }
};
