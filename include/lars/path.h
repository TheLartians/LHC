...
template <typename Vector> struct Conic : public Curve<Vector> {
  ...
  Conic(const Vector &control, const Vector &to)
      : Curve<Vector>(to), control(control) {}
  void accept(PathVisitor<Vector> *v) { v->visit(this); }
  void accept(ConstPathVisitor<Vector> *v) const { v->visit(this); }
};
template <typename Vector> struct Cubic : public Curve<Vector> {
  ...
  Cubic(const Vector &control_1, const Vector &control_2, const Vector &to)
      : Curve<Vector>(to), control_1(control_1), control_2(control_2) {}
  void accept(PathVisitor<Vector> *v) { v->visit(this); }
  void accept(ConstPathVisitor<Vector> *v) const { v->visit(this); }
};
} // namespace curves
...
class Path : public std::vector<std::unique_ptr<Curve<Vector>>> {
public:
  Vector start;
  ...
  Path(const Path &other) {
    PathVisitors::CopyVisitor<Vector> v(this);
    v.visit(&other);
  }
  ...
  void line_to(const Vector &point) { add_curve<curves::Line<Vector>>(point); }

  void conic_to(const Vector &control, const Vector &point) {
    add_curve<curves::Conic<Vector>>(control, point);
  }

  void cubic_to(const Vector &control_a, const Vector &control_b,
                const Vector &point) {
    add_curve<curves::Cubic<Vector>>(control_a, control_b, point);
  }

  std::vector<Vector> get_segmented_path_with_angle_tolerance(
      double angle_tolerance, double length_tolerance = 1e-5) const {
    PathVisitors::RecursiveSegmenter<Vector> v(angle_tolerance,
                                               length_tolerance);
    v.visit(this);
    return v.get_points();
  }
...
public:
  ...
  virtual void visit(const Path<Vector> *p) {
    copy->start = p->start;
    for (auto &c : *p)
      c->accept(this);
  }
  virtual void visit(const curves::Line<Vector> *c) {
    copy->template add_curve<curves::Line<Vector>>(*c);
  }
  virtual void visit(const curves::Conic<Vector> *c) {
    copy->template add_curve<curves::Conic<Vector>>(*c);
  }
  virtual void visit(const curves::Cubic<Vector> *c) {
    copy->template add_curve<curves::Cubic<Vector>>(*c);
  }
};

template <typename Vector>
class IncrementalSegmenter : public ConstPathVisitor<Vector> {
  ...
  Vector intermediate_cubic(Vector p0, Vector p1, Vector p2, Vector p3,
                            real t) {
...
public:
  ...
  void visit(const curves::Line<Vector> *e) { add_point(e->to); }
  ...
  std::vector<Vector> get_points() { return points; }
};

template <typename Vector>
class RecursiveSegmenter : public ConstPathVisitor<Vector> {

  std::vector<Vector> points;
  using Scalar = typename Vector::Scalar;

  Scalar angle_tolerance, cos_angle_tolerance_squared,
      cos_half_angle_tolerance_squared;
  ...
  void draw_conic_curve(const Vector &A, const Vector &C, const Vector &B) {
    ...
    auto length_test = [&]() {
      return l1_squared <= length_tolerance_squared &&
             l2_squared <= length_tolerance_squared;
    };
    auto angle_test = [&]() {
      return sign(dot) * cos_squared >= cos_angle_tolerance_squared;
    };

    if (length_test() || angle_test())
      add_point(B);
    else {
      Vector C1 = A + AC / 2;
      Vector C2 = B + BC / 2;
  ...
  void draw_cubic_curve(const Vector &A, const Vector &C1, const Vector &C2,
                        const Vector &B) {
    ...
    Scalar l1_squared = AC1.norm_squared(), l2_squared = BC2.norm_squared(),
           l3_squared = C1C2.norm_squared();
    ...
    auto length_test = [&]() {
      return l1_squared < length_tolerance_squared &&
             l2_squared < length_tolerance_squared &&
             l3_squared < length_tolerance_squared;
    };
    auto angle_test = [&]() {
      return sign(dot1) * cos1_squared > cos_half_angle_tolerance_squared &&
             sign(dot2) * cos2_squared > cos_half_angle_tolerance_squared;
    };
...
public:
  RecursiveSegmenter(Scalar angle_tolerance, Scalar length_tolerance)
      : angle_tolerance(angle_tolerance), length_tolerance(length_tolerance) {
    if (fabs(angle_tolerance) >= M_PI / 2)
      throw std::domain_error(
          "path segmentation angle tolerance must be smaller than pi/2");
    if (length_tolerance < 0)
      throw std::domain_error(
          "path segmentation length tolerance must be larger than zero");
    cos_angle_tolerance_squared = cos(angle_tolerance) * cos(angle_tolerance);
    cos_half_angle_tolerance_squared =
        cos(angle_tolerance / 2) * cos(angle_tolerance / 2);
  ...
  void visit(const curves::Line<Vector> *e) {
    ...
    if (d.dot(d) > length_tolerance_squared)
      add_point(e->to);
  }
...
template <typename Vector>
class BoundingBoxVisitor : public ConstPathVisitor<Vector>,
                           public BoundingBoxCreator<Vector> {
public:
  void visit(const Path<Vector> *e) {
...
public:
  ...
  void visit(curves::Line<Vector> *e) { e->to *= value; }
...
template <typename Vector>
class TouchingPointResolver : public PathVisitor<Vector> {
  ...
  void add_point(Vector &p) {
    if (points.find(p) != points.end()) {
      ...
      for (auto i : range(p.size())) {
        if ((*last_point)[i] > p2[i])
          p[i] = nextafter(p[i],
                           std::numeric_limits<typename Vector::Scalar>::max());
        else if ((*last_point)[i] < p2[i])
          p[i] = nextafter(p[i],
                           std::numeric_limits<typename Vector::Scalar>::min());
      }
      if (p == old)
        throw std::runtime_error("cannot move point");
      add_point(p);
    } else {
      points[p] = last_point;
      last_point = &p;
...
public:
  ...
  void visit(curves::Line<Vector> *e) { add_point(e->to); }

  void visit(curves::Conic<Vector> *e) { add_point(e->to); }

  void visit(curves::Cubic<Vector> *e) { add_point(e->to); }
};
