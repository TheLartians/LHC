...
public:
  ...
  Iterator begin() const {
    Iterator it;
    it.it = index_ranges().begin();
    return it;
  }
  Iterator end() const {
    Iterator it;
    it.it = index_ranges().end();
    return it;
  }
  size_t count() const {
    size_t res = 0;
    for (auto r : this->index_ranges())
  ...
  Index min() const {
    assert(index_ranges().size() > 0);
    return index_ranges().front().first;
  }
  Index max() const {
    assert(index_ranges().size() > 0);
    return index_ranges().back().last;
  }

  void add_indices(const CompressedIndexSet<Index> &other) {
    for (auto &r : other.index_ranges())
      insert_index_range(r.first, r.last);
  }
  void remove_indices(const CompressedIndexSet<Index> &other) {
    for (auto &r : other.index_ranges())
      remove_index_range(r.first, r.last);
  }
  void unite(const CompressedIndexSet<Index> &other) { add_indices(other); }
  void intersect(const CompressedIndexSet<Index> &other) {
    CompressedIndexSet<Index> tmp;
    tmp.insert_index_range(min(), max());
    tmp.remove_indices(other);
    remove_indices(tmp);
  }
};

template <class Index>
void CompressedIndexSet<Index>::insert_index_range(Index first, Index last) {
  assert(last >= first);
...
template <class Index>
void CompressedIndexSet<Index>::remove_index_range(Index first, Index last) {
  assert(last >= first);
...
std::basic_ostream<Char, Traits> &
operator<<(std::basic_ostream<Char, Traits> &stream,
           const CompressedIndexSet<Index> &indices) {
  stream << '{';
  for (auto &r : indices.index_ranges()) {
    stream << r;
    if (&r != &indices.index_ranges().back())
      stream << ',';
  }
  stream << '}';
  return stream;
